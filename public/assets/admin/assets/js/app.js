...
$(document).ready(function () {
    $('.btn-img').click(function (e) {
        e.preventDefault();
        $(this).siblings('.inpt-img-upload')[0].click();
    })

    $('.inpt-img-upload').change(function (e) {
        ...
        for (var i = 0; i < e.originalEvent.srcElement.files.length; i++) {
            ...
            var ini = $(this);
            var kondisi = $(this).attr('con');
            reader.onloadend = function () {
                var filename = e.target.files[0].name;
                var src = reader.result;
                // ini.siblings('img').attr('src', src);
                // $('#nama-sertifikat').attr("placeholder", filename);

                if (kondisi = "upload") {
                    ini.siblings('img').attr('src', src);

                    // Disini Tulis script upload img

                }
                else if (kondisi = "input") {
                    ini.siblings('img').attr('src', src);
                }
            }
